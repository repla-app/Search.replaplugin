        ...
        def self.test_file_hashes(files_hash, test_files_hash)
          ...
          test_files_hash.keys.each do |file_path|
            ...
            if test_file.file_path != file.file_path
              puts "File path #{file.file_path} should match "\
                "#{test_file.file_path}."
              return false
            end

            if test_file.display_file_path != file.display_file_path
              puts "Display file path #{file.display_file_path} should match "\
                "#{test_file.display_file_path}."
              return false
            end

            test_file.lines.zip(file.lines).each do |test_line, line|
              if test_line.number != line.number
                puts "Line number #{line.number} should match "\
                  "#{test_line.number}."
                return false
              end

              if test_line.matches.count != line.matches.count
                puts "Match count #{line.matches.count} should match "\
                  "#{test_line.matches.count}."
                return false
              end
